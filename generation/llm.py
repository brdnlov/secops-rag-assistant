...
class Generator:
    ...
    def _system_prompt(self):
        return (
            ...
            "Every factual claim must carry a citation. In particular, when the "
            "answer relates two or more sources (e.g. a GDPR article and a NIST "
            "control), state the comparison explicitly and attach a citation next "
            "to the claim about EACH source — do not leave the bridging statement "
            "uncited. "
            "Only cite a source that is actually present in the context. "
            "If the question asks about a source that is NOT in the provided "
            "context (e.g. it names a control or article whose passage was not "
            "retrieved), say clearly which source is missing from the context "
            "before answering the part you can — do not invent its content or "
            "claim it says something the context does not support. "
            "If the context describes two requirements but does not explicitly "
            "state how they relate, do not assert a definitive relationship; "
            "describe each from the source with its citation and note that the "
            "sources do not state the relationship explicitly. "
